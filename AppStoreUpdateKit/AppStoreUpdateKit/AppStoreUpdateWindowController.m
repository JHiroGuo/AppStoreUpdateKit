...
-(void)__initializeAppStoreUpdateWindowController:(AppStoreUpdateUIConfigure *)configure {
    ...
    _lbName = [[NSTextField alloc] initWithFrame:NSMakeRect(0, NSMinY(_ivIcon.frame) - 30, NSWidth(rctWindow), 28)];
    ...
    [_lbName setFont:[NSFont fontWithName:@"Helvetica Neue Light" size:22]];
    [_lbName setAlignment:NSCenterTextAlignment];
    [_lbName setStringValue:[_appObj appName]];
    [window.contentView addSubview:_lbName];
    
    _lbVersion = [[NSTextField alloc] initWithFrame:NSMakeRect((NSWidth(rctWindow) - 110) / 2, NSMinY(_lbName.frame) - 20, 110, 19)];
    ...
    [_lbVersion setFont:[NSFont fontWithName:@"Helvetica Neue" size:14]];
    ...
    _lbReleaseNotes = [[NSTextField alloc] initWithFrame:NSMakeRect((NSWidth(rctWindow) - 250) / 2,NSMinY(_lbName.frame) - 180, 250, 155)];
    ...
    [[_lbReleaseNotes cell] setLineBreakMode:NSLineBreakByCharWrapping];
    [[_lbReleaseNotes cell] setTruncatesLastVisibleLine:YES];
    [window.contentView addSubview:_lbReleaseNotes];
    
    _btnUpdate =  [[ASUButton alloc] initWithFrame:NSMakeRect((NSWidth(rctWindow) - 250) / 2, NSMinY(_lbReleaseNotes.frame) - 60, 250, 45)];
    ...
    [_btnUpdate setFont:[NSFont fontWithName:@"Helvetica Neue Medium" size:20]];
