...
-(BOOL)checkAppUpdate:(AppStoreUpdateAppObject *)appObj{
    ...
    NSError *error;
    NSData *response = [NSURLConnection sendSynchronousRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@,%@",strRequest,[appObj productID]]]] returningResponse:nil error:nil];
    if (nil == response) {
        return bRslt;
    }
